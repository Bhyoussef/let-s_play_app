...
class PrivateMatches extends StatelessWidget {
  final SportCategoryModel sport;
  const PrivateMatches({Key? key, required this.sport}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
              child: BlocBuilder<PrivateMatchesCubit, PrivateMatchesState>(
              builder: (context, state) {
              if (state.mainStatus == MainStatus.loaded) {
                return ListView.builder(
                  itemCount: state.privatePlaygrounds!.length,
                  scrollDirection: Axis.vertical,
                  shrinkWrap: true,
                  itemBuilder: (context, index) {
                    final field = state.privatePlaygrounds![index];
                    return GestureDetector(
                      onTap: () {
                        Navigator.pushNamed(context, RouteList.playgroundDetailScreen, arguments: {
                          'playgroundId': field.id,
                          'publicOrPrivate': PublicPrivateType.private
                        });
                      },
                      child: Container(
